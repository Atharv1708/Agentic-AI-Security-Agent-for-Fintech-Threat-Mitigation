...
# --- LLM Anomaly Detection (Robust Parsing, Unchanged logic) ---
...
async def detect_llm_anomaly(event: EventData) -> Optional[Dict]:
    ...
    system_prompt = (
        ...
        "'attack_type' (string, e.g., 'SQL Injection', 'XSS', 'Benign Activity'), "
        "'severity' (string: 'LOW', 'MEDIUM', 'HIGH', 'CRITICAL', 'NONE'), and "
    ...
    try:
        ...
        required_keys = {"is_malicious", "attack_type", "severity", "reason"}
        ...
        if is_malicious:
            ...
            logger.info(
                f"LLM detected malicious: Type='{attack_type}', Severity='{severity}'")
            return {"is_attack": True, "attack_type": attack_type, "reason": reason, "severity": severity, "confidence": 0.70}
        else:
        ...
        # RequestError does not always expose a response; only HTTPStatusError has .response
        status = "N/A"
        if isinstance(e, httpx.HTTPStatusError) and getattr(e, "response", None) is not None:
            try:
                ...
                status = "N/A"
        error_msg = f"LLM request failed: Status={status}, Error='{e}'"
...
async def detect_card_testing(event: EventData) -> Optional[Dict]:
    if "payment_failure" not in event.event_type:
        return None
    now = time.time()
    ip = event.source_ip
...
async def detect_dos_ddos(event: EventData) -> Optional[Dict]:
    ...
    if ip:
        ...
        ip_log.append(now)
        # --- FIX: Access float directly ---
        while ip_log and ip_log[0] < cutoff:  # REMOVED extra [0]
            ip_log.popleft()
        # --- End FIX ---
    ...
    if global_request_count > settings.DOS_ATTACK_THRESHOLD:
        ...
        reason = f"{global_request_count} req/{settings.TIME_WINDOW_SECONDS}s from {active_ips_count} IPs."
...
DETECTOR_PIPELINE: List[Callable[[EventData], Awaitable[Optional[Dict]]]] = [
    detect_dos_ddos, detect_card_testing, detect_llm_anomaly]
